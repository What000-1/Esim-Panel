...
- 新增卡片可从 BankNav「海外电话卡」目录选择 69 种卡，自动填入其列出的保号操作、周期和初始到期日。目录排除了「平台卡」以及实体 eSIM 写卡工具。未给出明确周期的卡会提示手动填写；可直接修改所有预填内容。
- BankNav 明确标注「无需保号」的卡可保存为无到期日卡片，不参与保号提醒或续期。One NZ 按初始 4 年、此后每年充值的不同周期预填。目录为 2026-09-24 的静态快照，使用前请通过表单中的来源链接核对运营商最新规则。
...
1. 点击「新增 eSIM」，在「海外电话卡」中按地区选择品牌。例如选「英国 · giffgaff」，表单会填入 180 天周期和余额变动的保号要求。
