...
<body class="text-gray-800 font-sans p-4 md:p-8 relative">
        ...
        <div class="flex flex-col md:flex-row justify-between items-center mb-10 border-b border-white/50 pb-6 gap-4">
            ...
            <div class="flex gap-3 items-center flex-wrap justify-center">
                ...
                <div class="relative group inline-block">
                    <button class="bg-indigo-50 hover:bg-indigo-100 text-indigo-600 px-4 py-2.5 rounded-full font-bold shadow-sm transition-colors flex items-center gap-2 border border-indigo-200">
                        <i class="fa-solid fa-cloud"></i> <span class="hidden sm:inline">备份与恢复</span>
                    </button>
                    <div class="absolute right-0 mt-2 w-36 bg-white rounded-xl shadow-lg border border-gray-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
                        <button onclick="openExportModal()" class="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-indigo-50 hover:text-indigo-600 rounded-t-xl flex items-center gap-2"><i class="fa-solid fa-file-export w-4 text-center"></i> 导出备份</button>
                        <button onclick="openImportModal()" class="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-indigo-50 hover:text-indigo-600 rounded-b-xl flex items-center gap-2"><i class="fa-solid fa-file-import w-4 text-center"></i> 导入备份</button>
                    </div>
                </div>
    ...
    <!-- 导出模态框 -->
    <div id="exportModal" class="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4">
        <div class="glass-card w-full max-w-sm rounded-2xl p-6 shadow-2xl relative transition-all duration-300 transform scale-95 opacity-0" id="exportModalContent">
            <button onclick="closeExportModal()" class="absolute top-4 right-4 text-gray-500 hover:text-red-500 text-xl">
                <i class="fa-solid fa-xmark"></i>
            </button>
            <h3 class="text-xl font-bold text-gray-900 mb-4 flex items-center gap-2">
                <i class="fa-solid fa-shield-halved text-indigo-600"></i> 加密导出
            </h3>
            <p class="text-sm text-gray-600 mb-4">请设置一个密码，导出的备份文件必须输入此密码才能打开或导入。</p>
            <div class="mb-6">
                <input type="password" id="exportPassword" placeholder="设置加密密码" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white/80">
            </div>
            <button onclick="exportData()" id="exportBtn" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-4 rounded-xl shadow-md transition-colors flex items-center justify-center gap-2">
                <i class="fa-solid fa-download"></i> 下载加密备份
            </button>
        </div>
    </div>

    <!-- 导入模态框 -->
    <div id="importModal" class="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4">
        <div class="glass-card w-full max-w-md rounded-2xl p-6 shadow-2xl relative transition-all duration-300 transform scale-95 opacity-0" id="importModalContent">
            <button onclick="closeImportModal()" class="absolute top-4 right-4 text-gray-500 hover:text-red-500 text-xl">
                <i class="fa-solid fa-xmark"></i>
            </button>
            <h3 class="text-xl font-bold text-gray-900 mb-4 flex items-center gap-2">
                <i class="fa-solid fa-file-import text-indigo-600"></i> 导入备份
            </h3>
            <p class="text-sm text-gray-600 mb-4">请选择之前导出的加密 JSON 备份文件，并输入当时的密码。</p>
            
            <div class="mb-4">
                <input type="file" id="importFile" accept=".json" class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 cursor-pointer">
            </div>
            <div class="mb-4">
                <input type="password" id="importPassword" placeholder="输入解密密码" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white/80">
            </div>
            <div class="mb-6">
                <label class="block text-gray-700 text-sm font-bold mb-2">遇到重复数据时（ID相同）：</label>
                <div class="flex gap-4">
                    <label class="flex items-center gap-2 cursor-pointer text-sm">
                        <input type="radio" name="importMode" value="merge" checked class="w-4 h-4 text-indigo-600 focus:ring-indigo-500">
                        覆盖同ID记录并追加新记录 (合并)
                    </label>
                </div>
                <div class="flex gap-4 mt-2">
                    <label class="flex items-center gap-2 cursor-pointer text-sm">
                        <input type="radio" name="importMode" value="overwrite" class="w-4 h-4 text-indigo-600 focus:ring-indigo-500">
                        清空当前所有数据，完全替换为备份
                    </label>
                </div>
            </div>
            <button onclick="importData()" id="importBtn" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-4 rounded-xl shadow-md transition-colors flex items-center justify-center gap-2">
                <i class="fa-solid fa-cloud-arrow-up"></i> 解密并导入
            </button>
        </div>
    </div>
    ...
    <script>
        ...
        async function importData() {
            ...
            if (!fileInput.files || fileInput.files.length === 0) return alert("请选择备份文件！");
            ...
            try {
                ...
                // 发送给后端
                const response = await fetch(WORKER_API_URL + '/import', {
                    method: 'POST',
                    headers: getAuthHeaders(),
...
export default {
    async fetch(request, env, ctx) {
        ...
        // ================= 导入数据 API =================
        if (path === "/api/esims/import" && request.method === "POST") {
            ...
            try {
                ...
                return new Response(JSON.stringify({ success: false, message: "处理失败" }), { status: 500, headers: corsHeaders });
            }
        }
